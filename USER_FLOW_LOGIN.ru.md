...
Роутер: **`BrowserRouter`**, **`basename={import.meta.env.BASE_URL}`**; пути **относительно приложения**.
...
**Deep link на вкладки нет:** есть только **`/login`**; **`РЕГИСТРАЦИЯ`** — вторая вкладка того же компонента.
...
Как при регистрации: **`location.state.from`** переопределяет **`/editor`** по умолчанию (`LoginPage.tsx`).

### 7.3 Гость открыл редактор, затем решил войти

1. Выбор **Блокн0т** на onboarding → **`/editor`**, **`isGuest`** true.
2. **«ВОЙТИ →»** вызывает **`onLogin`** → **`/login`** с **`from: /editor`**.
3. Успешный вход → **`/editor`** с **`token`**; **`isGuest`** false.
...
- [ ] Гость **note** → **`/editor`** → **«ВОЙТИ →»** → **`/login`** → вход → снова **`/editor`** с сессией.
