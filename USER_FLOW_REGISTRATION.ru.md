...
### 7.1. Цель редиректа после регистрации

Как и после входа по форме (`LoginPage.tsx`): если в **`location.state`** передан объект **`from`** (например после редиректа с **`EditorPage`** или **`AdminPage`**), **`navigate(target, { replace: true })`** ведёт на **`from.pathname` + `from.search`**, а не на **`/editor`** по умолчанию. Если **`from`** отсутствует, цель — **`/editor`**.

### 7.2. Регистрация после онбординга со структурированным профилем

Речь о выборе **структурированного** режима на **`/`** (не гостевой **Блокнот** / **`note`**). Допустимые пары «подпись в UI → **`mode`** в **`ow_profile`**»:
...
3. Заполняет **логин**, **email**, **пароль** и отправляет форму (**`СОЗДАТЬ АККАУНТ`** или **Enter** в полях логина/пароля — шаги в **§6**).
4. При успешном **`POST /api/auth/register`** Redux получает **`token`** и **`user`**, токен пишется в **`ow_token`**; вызывается **`onLogin()`** → переход на **`/editor`** (или путь из **`from`**), сессия активна, профиль онбординга уже лежит в **`ow_profile`**.

### 7.3. Ограничения UI

**Отдельного мастера, модального окна или маршрута `/register` нет** — только вторая вкладка **`РЕГИСТРАЦИЯ`** той же карточки **`Login`** на **`/login`**.
...
- [ ] На **`/`** выбрать **Сценарий**, **Пьесу**, **Видео** или **Медиа** (любой структурированный режим, **`mode` ≠ `note`**) → убедиться, что открыт **`/login`** с **`state.from`** на редактор → вкладка **`РЕГИСТРАЦИЯ`** → валидные данные → **`/editor`**, **`ow_token`** задан, **`EditorScreen`** получает сохранённый **`ow_profile`**.
