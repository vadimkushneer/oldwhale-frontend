...
- **Service worker / PWA:** `npm run e2e:serve` собирает с `VITE_PWA_DISABLED=1`, поэтому при тестах service worker не регистрируется и `manifest.webmanifest` не эмитится — визуальные эталоны не видят PWA-ассетов и апдейт-промптов. Чтобы проверить именно PWA, соберите без этого флага и отдайте `dist/` любым статическим сервером (например, `npx serve dist -p 4173`).
